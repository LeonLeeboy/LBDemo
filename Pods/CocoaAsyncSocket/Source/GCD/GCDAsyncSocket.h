...
/**
 * GCDAsyncSocket uses the standard delegate paradigm（范例）,
    GCDSsynSocket 用了标准的代理范例。
 * but executes all delegate callbacks on a given delegate dispatch queue.
    但是执行所有的代理回调是在一个作为代理的dispatchqueue中。
 * This allows for maximum concurrency, while at the same time providing easy thread safety.
 * 这个保证了最大的并发量，同时提供一个简单的线程安全。
 * You MUST set a delegate AND delegate dispatch queue before attempting to
 * use the socket, or you will get an error.
 * 你必须设置一个代理，并且设置一个dispatch queue 的代理，在你用socket 之前，否则你会get一个错误。
 * The socket queue is optional.
   socket queue 是可选的
 * If you pass NULL, GCDAsyncSocket will automatically create it's own socket queue.
   如果你传一个NULL，GCDAsyncSocket将会自动创建一个自己的socket queue。
 * If you choose to provide a socket queue, the socket queue must not be a concurrent queue.
    如果你提供一个socket queue，这个socket queue 必须不能为并发队列。
 * If you choose to provide a socket queue, and the socket queue has a configured target queue,
    如果你选择自己提供一个socket queue，并且socket queue 有一个明确的target queue。
 * then please see the discussion for the method markSocketQueueTargetQueue.
 *  然后看方法markSocketQueueTargetqueue 的 讨论
 * The delegate queue and socket queue can optionally be the same.
    delegate queue 和 socket queue 可以相同。
**/
- (instancetype)init;
 ...
 * 如果你正在delegate的dealloc 的方法里面将delegate 设置为nil ，你可能需要用到下面的同步方法。
**/
 ...
 * This data is not used internally（内部） by socket in any way.
**/
 ...
 * 告诉socket 开始监听和接受连接在给定port上。当一个connection被接受了。一个GCDAsyncsocket 对象将会产生然去处理他。
 * 然后 方法 socket:didAcceptNewSocket: 将会被调用。
